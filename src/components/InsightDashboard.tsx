...
import { 
  TrendingUp, FileText, Compass, CirclePlus, Sparkles, 
  Search, Bell, ChevronDown, User, LayoutDashboard, MessageSquare, 
  History, Download, Zap, BookOpen, AlertCircle, ShieldCheck,
  AlertTriangle, Lightbulb, Settings, Plus, Edit, Trash2
} from "lucide-react";
...
export default function InsightDashboard({ services, problems, onAddProblem, onTabChange, userRole }: InsightDashboardProps) {
  ...
  const [problemFilterCat, setProblemFilterCat] = useState("Semua");
  
  // CMS states
  const [cmsServices, setCmsServices] = useState<CampusService[]>(services);
  const [cmsSearch, setCmsSearch] = useState("");
  const [cmsFilterCat, setCmsFilterCat] = useState("Semua");

  // Sync cmsServices with services prop
  useEffect(() => {
  ...
  const handleNavigation = (tabName: string) => {
    ...
    if (onTabChange) {
      ...
      else if (tabName === "Knowledge Base CMS") onTabChange("insight");
    }
  };
      ...
      <aside className="w-64 bg-[#0F172A] border-r border-slate-800 flex flex-col z-20 shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 custom-scrollbar">
          ...
          {[
            ...
            { name: "Knowledge Base CMS", icon: Settings },
          ].map((item) => (
            <button
              key={item.name}
              onClick={() => handleNavigation(item.name)}
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            {activeMenu === "Insight Admin" && (
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  {/* Main Chart: Kategori Teratas (5 cols) */}
                  ...
                  <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="relative overflow-hidden bg-gradient-to-br from-teal-900/40 to-slate-900 border border-teal-500/30 rounded-2xl p-6 shadow-[0_0_30px_rgba(20,184,166,0.05)]">
                      ...
                      <ul className="space-y-4 relative z-10">
                        ...
                        <li className="flex gap-3 items-start">
                          <div className="h-1.5 w-1.5 rounded-full bg-teal-450 mt-1.5 shrink-0 shadow-[0_0_5px_currentColor]"></div>
                          <p className="text-xs text-slate-300 leading-relaxed">
                    ...
                    <div className="bg-[#1E293B]/60 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => alert("Fitur Tambah FAQ akan tersedia di versi produksi.")} className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 transition-colors text-slate-300 hover:text-white cursor-pointer">
                          <CirclePlus className="h-4 w-4 mb-2 text-teal-400" />
                          <span className="text-[10px] font-semibold">Tambah FAQ</span>
                        </button>
                        <button onClick={() => alert("Fitur Update Base akan tersedia di versi produksi.")} className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 transition-colors text-slate-300 hover:text-white cursor-pointer">
                          <BookOpen className="h-4 w-4 mb-2 text-rose-400" />
                          <span className="text-[10px] font-semibold">Update Base</span>
                        </button>
                        <button onClick={() => alert("Laporan Bulanan berhasil diexport!")} className="flex flex-col items-center justify-center p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 transition-colors text-slate-300 hover:text-white col-span-2 cursor-pointer">
                ...
                {/* Narrative 5: Campus Problem Bank Preview */}
                <div className="bg-[#1E293B]/60 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button 
                      onClick={() => setActiveMenu("Problem Bank")}
                      className="text-xs text-rose-450 hover:text-rose-350 font-bold transition-colors border-b border-transparent hover:border-rose-450 cursor-pointer"
                    >
                    ...
                    {problems.slice(0,3).map((prob) => (
                      <div key={prob.id} className="bg-slate-800/40 border border-slate-700 hover:border-rose-500/50 rounded-xl p-4 transition-all group flex flex-col justify-between cursor-pointer">
                        ...
                        <div className="mt-4 flex items-center justify-between">
                          ...
                          <button 
                            onClick={() => alert(`Inovasi Challenge "${prob.title}" berhasil di-publish!`)}
                            className="text-[10px] font-bold text-teal-400 opacity-80 group-hover:opacity-100 bg-teal-400/10 px-2 py-1 rounded transition-opacity cursor-pointer border border-teal-400/20"
                          >
                            Jadikan Challenge
                          </button>
                        </div>
            ...
            {activeMenu === "Problem Bank" && (
              <div className="space-y-6 text-left">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <h3 className="text-base font-bold text-white flex items-center gap-2">
                      ...
                      Campus Problem Bank Repository
                    </h3>
                    <p className="text-xs text-slate-400 mt-1">
                      Kumpulan permasalahan operasional dan aduan kampus yang dapat dijadikan ide tantangan inovasi bagi mahasiswa.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-48 bg-slate-905 border border-slate-700 rounded-full py-1.5 pl-8 pr-3.5 text-xs text-white focus:outline-hidden focus:border-teal-500 focus:ring-1 focus:ring-teal-500/50 transition-all placeholder:text-slate-500"
                      />
                    </div>
                    
                    <select
                      value={problemFilterCat}
                      onChange={(e) => setProblemFilterCat(e.target.value)}
                      className="bg-slate-905 border border-slate-700 rounded-full py-1.5 px-3.5 text-xs text-slate-300 focus:outline-hidden appearance-none cursor-pointer"
                    >
                ...
                {/* Add Custom Problem Form (Simulation) */}
                <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5">
                  <h4 className="text-xs font-bold text-white mb-3 flex items-center gap-1.5">
                    <CirclePlus className="h-4 w-4 text-teal-400" /> Laporkan Masalah Baru ke Bank Masalah
                  </h4>
                  <form onSubmit={handleAddSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                    <div className="md:col-span-7">
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Judul Masalah</label>
                      <input
                        type="text"
                        placeholder="Contoh: Antrean validasi berkas wisuda lambat..."
                        value={newTitle}
                        onChange={(e) => setNewTitle(e.target.value)}
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl py-2 px-3 text-xs text-white focus:outline-hidden focus:border-teal-500"
                      />
                    </div>
                    <div className="md:col-span-3">
                      ...
                      <select
                        ...
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl py-2 px-3 text-xs text-slate-350 focus:outline-hidden appearance-none cursor-pointer"
                      >
                        <option value="Akademik">Akademik</option>
                        <option value="IT & Jaringan">IT & Jaringan</option>
                        <option value="Keuangan">Keuangan</option>
                    ...
                    <div className="md:col-span-2">
                      <button
                        type="submit"
                        className="w-full py-2.5 bg-teal-500 hover:bg-teal-450 text-slate-900 font-bold text-xs rounded-xl transition-all cursor-pointer shadow-sm active:scale-95"
                      >
                        Tambah
                      </button>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {problems
                    .filter((p) => {
                      ...
                      const matchCat = problemFilterCat === "Semua" || p.category === problemFilterCat;
                      return matchSearch && matchCat;
                    })
                    .map((prob) => {
                      const isKritis = prob.status === "Kritis";
                      const isReview = prob.status === "Dalam Peninjauan";
                      return (
                        <div
                          key={prob.id}
                          className="bg-[#1E293B]/60 backdrop-blur-md border border-slate-800 rounded-2xl p-5 flex flex-col justify-between hover:border-slate-700 transition-all group"
                        >
                          <div>
                            <div className="flex items-center justify-between mb-3">
                              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider bg-slate-900 px-2 py-0.5 rounded border border-slate-800">
                                {prob.category}
                              </span>
                            ...
                            <h4 className="text-sm font-bold text-white leading-snug group-hover:text-teal-450 transition-colors">
                              {prob.title}
                            </h4>
                            <p className="text-[10.5px] text-slate-450 mt-2.5">
                              Dilaporkan sebanyak <span className="text-white font-bold">{prob.reportedCount} kali</span> oleh civitas akademika.
                            </p>
                          </div>

                          <div className="mt-4 pt-4 border-t border-slate-800/80 flex justify-between items-center">
                            <span className="text-[9px] text-slate-500 font-mono">ID: {prob.id}</span>
                            <button
                              onClick={() => {
                                alert(`Tantangan Inovasi "${prob.title}" berhasil diterbitkan ke Hackathon Smart Campus!`);
                              }}
                              className="text-[10px] font-bold text-teal-400 opacity-90 group-hover:opacity-100 bg-teal-400/10 px-3 py-1.5 rounded-lg border border-teal-400/20 hover:bg-teal-400 hover:text-slate-900 transition-all cursor-pointer"
                            >
                              Jadikan Challenge
                            </button>
                          </div>
            ...
            {activeMenu === "Knowledge Base CMS" && (
              <div className="space-y-6 text-left">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <h3 className="text-base font-bold text-white flex items-center gap-2">
                      ...
                      Knowledge Base CMS Layanan
                    </h3>
                    <p className="text-xs text-slate-400 mt-1">
                      Kelola daftar layanan resmi, perbarui langkah-langkah prosedur, dan sesuaikan rekomendasi AI.
                    </p>
                  </div>
                  
                  <div className="flex gap-2">
                    <button
                      onClick={() => alert("Fitur Tambah Layanan baru akan diintegrasikan pada sistem produksi.")}
                      className="inline-flex items-center gap-1.5 bg-teal-500 hover:bg-teal-450 text-slate-900 py-2 px-4 rounded-full text-xs font-bold transition-all cursor-pointer"
                    >
                ...
                {/* Filter and Search */}
                <div className="bg-slate-900/30 border border-slate-800 p-4 rounded-2xl flex flex-col sm:flex-row gap-3 justify-between items-center">
                  <div className="relative w-full sm:w-64">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-500" />
                    <input
                      type="text"
                      placeholder="Cari berkas layanan..."
                      value={cmsSearch}
                      onChange={(e) => setCmsSearch(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-850 rounded-xl py-2 pl-8 pr-3 text-xs text-white focus:outline-hidden placeholder:text-slate-500"
                    />
                  </div>
                  <select
                    value={cmsFilterCat}
                    onChange={(e) => setCmsFilterCat(e.target.value)}
                    className="bg-slate-950 border border-slate-850 rounded-xl py-2 px-3 text-xs text-slate-350 focus:outline-hidden appearance-none cursor-pointer w-full sm:w-auto"
                  >
                    <option value="Semua">Semua Kategori</option>
                    <option value="Akademik">Akademik</option>
                ...
                {/* CMS Services Table */}
                <div className="bg-[#1E293B]/60 backdrop-blur-md border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[700px]">
                      <thead>
                        <tr className="bg-slate-950/60 border-b border-slate-850 text-slate-400 text-[10px] font-extrabold uppercase tracking-wider">
                          <th className="py-3 px-6">Nama Layanan</th>
                          <th className="py-3 px-4">Kategori</th>
                          <th className="py-3 px-4">Status KB</th>
                      ...
                      <tbody className="divide-y divide-slate-850 text-xs text-slate-300">
                        {cmsServices
                          .filter((svc) => {
                            const matchSearch = svc.title.toLowerCase().includes(cmsSearch.toLowerCase());
                            const matchCat = cmsFilterCat === "Semua" || svc.category === cmsFilterCat;
                            return matchSearch && matchCat;
                          })
                          .map((svc, sIdx) => {
                            const isOdd = sIdx % 2 !== 0;
                            const statuses = ["Terverifikasi", "Perlu Update", "Draft"] as const;
                            const status = statuses[sIdx % 3];
                            
                            return (
                              <tr key={svc.id} className={`${isOdd ? "bg-slate-900/20" : "bg-transparent"} hover:bg-slate-800/40 transition-colors`}>
                                <td className="py-4 px-6">
                                  <p className="font-extrabold text-slate-100">{svc.title}</p>
                                  <p className="text-[10px] text-slate-500 mt-1 line-clamp-1">{svc.description}</p>
                                </td>
                                <td className="py-4 px-4 font-semibold text-slate-400">{svc.category}</td>
                                <td className="py-4 px-4">
                                  <span className={`inline-flex items-center rounded-md px-2.5 py-0.5 text-[9px] font-bold border ${
                                ...
                                <td className="py-4 px-4 font-mono text-[10px] text-slate-450">
                                  {sIdx === 0 ? "Hari ini, 19:40" : `${(sIdx + 2) * 3} Mei 2026`}
                                </td>
                                <td className="py-4 px-6 text-center space-x-2">
                                  <button
                                    onClick={() => alert(`Status validasi untuk "${svc.title}" telah diperbarui.`)}
                                    className="bg-slate-800 hover:bg-slate-700 hover:text-white border border-slate-700 py-1.5 px-2.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer"
                                  >
                                    Validasi
                                  </button>
                                  <button
                                    onClick={() => alert("Fitur edit KB akan tersedia di versi produksi.")}
                                    className="bg-teal-500/10 text-teal-400 hover:bg-teal-500 hover:text-slate-900 border border-teal-500/20 py-1.5 px-2.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer"
                                  >
                                    Edit
                                  </button>
                                </td>
                ...
                {/* CMS Disclaimer Footer Callout */}
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 text-[10.5px] text-slate-450 leading-relaxed">
                  💡 <span className="font-bold text-slate-300">Catatan Sandbox:</span> Fitur ini merupakan pratinjau (*preview*) halaman pengelola agar unit layanan resmi kampus dapat memperbarui informasi prosedur dan rekomendasi AI secara mandiri tanpa memerlukan bantuan developer.
                </div>
              </div>
            )}
          </div>
